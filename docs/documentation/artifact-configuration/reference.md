...
|-------------------|----------|--------------------------------------|--------------------------------------------------------
| `hash-algorithm`  | `sha256` | `sha1`, `sha256`, `sha384`, `sha512` | Hash algorithm used to calculate the signature digest.<br><span style="font-family: 'Line Awesome Free'; font-weight: 800;">&#61530;</span> `<appx-file>` and `<msix-file>` don't support `sha1` and require the `hash-algorithm` to match the one specified in the `AppxBlockMap.xml` file.
